    ...
    setupLaptopWebSocketHandlers(ws, {
      ...
      onMobileDisconnected: (mobileId) => {
        ...
        if (pairedMobileId === mobileId) {
          pairedMobileId = null;
          updateConnectionStatus('Paired mobile device disconnected', 'not-connected');
          resetData();
        }
      },
      onPairRequest: (mobileId) => {
        console.log('Received pairing request from mobile device:', mobileId);
        if (confirm(`Mobile device (${mobileId.substring(0, 8)}...) wants to pair with your laptop. Accept?`)) {
...
function setupCharts() {
  // Eye Movement Chart
  const eyeMovementCtx = document.getElementById('eye-movement-chart').getContext('2d');
...
function addAvailableMobile(mobileId) {
  ...
  if (mobilesList.querySelector('li')?.textContent.includes('No mobile devices available')) {
    mobilesList.innerHTML = '';
  }
...
function addToConnectedDevices(deviceId) {
  ...
  if (connectedDevicesList.querySelector('li')?.textContent.includes('No devices connected')) {
    connectedDevicesList.innerHTML = '';
  }
  
  // Check if device is already in the list
  if (Array.from(connectedDevicesList.querySelectorAll('li')).some(li => li.dataset.id === deviceId)) {
    return;
  }
...
function processEyeTrackingData(data) {
  ...
  if (eyeTrackingData.length > 60) { // Last 60 data points (1 minute at 1 Hz)
  ...
  if (eyeMovementChart) {
    ...
    eyeMovementChart.data.datasets[0].data = eyeTrackingData.map(d => d.blinkRate);
    eyeMovementChart.data.datasets[1].data = eyeTrackingData.map(d => d.saccadeVelocity);
...
function processHeartRateData(data) {
  ...
  // Limit the data array size
  if (heartRateData.length > 30) { // Last 30 data points (1 minute at 0.5 Hz)
    heartRateData.shift();
  }
  ...
  if (heartRateChart) {
    const timestamps = heartRateData.map((_, index) => {
      const time = new Date();
      time.setSeconds(time.getSeconds() - (heartRateData.length - index - 1) * 2);
      return time.toLocaleTimeString('en-US', { hour12: false, hour: '2-digit', minute: '2-digit', second: '2-digit' });
    });
...
function updateEyeTrackingMetrics(latestData) {
  ...
  blinkRateElement.textContent = `${latestData.blinkRate.toFixed(1)} blinks/min`;
  gazeDurationElement.textContent = `${latestData.gazeDuration.toFixed(1)} seconds`;
...
function updateHeartRateMetrics() {
  ...
  // Average heart rate
  const avgHR = heartRateData.reduce((sum, data) => sum + data.bpm, 0) / heartRateData.length;
...
function updateCombinedMetrics() {
  ...
  const hrStressFactor = Math.max(0, Math.min(1, (latestHeartData.bpm - 60) / 40)); // Normalize HR from 60-100 to 0-1
  const blinkStressFactor = Math.min(latestEyeData.blinkRate / 20, 1); // Normalize to 0-1
  const stressLevel = Math.round((hrStressFactor * 0.7 + blinkStressFactor * 0.3) * 100);
  ...
  const attentionScore = Math.round((gazeFactor * 0.6 + saccadeFactor * 0.4) * 100);
  ...
  if (combinedChart) {
    ...
    for (let i = 0; i < dataPoints; i++) {
      ...
      const stressValue = Math.round((hrFactor * 0.7 + blinkFactor * 0.3) * 100);
      
      const gazeFactor = Math.min(eyeData.gazeDuration / 5, 1);
      const saccadeFactor = Math.max(0, 1 - eyeData.saccadeVelocity / 100);
      const attentionValue = Math.round((gazeFactor * 0.6 + saccadeFactor * 0.4) * 100);
      
      // Create timestamp
      const time = new Date();
      time.setSeconds(time.getSeconds() - (dataPoints - i - 1) * 2);
      timestamps.push(time.toLocaleTimeString('en-US', { hour12: false, hour: '2-digit', minute: '2-digit', second: '2-digit' }));
