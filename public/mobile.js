...
async function startCamera() {
  try {
    ...
    // Update UI
    document.getElementById('toggle-camera').innerHTML = '<i data-feather="video-off"></i> Stop Camera';
    feather.replace();
...
function stopCamera() {
  if (cameraStream) {
    ...
    // Update UI
    document.getElementById('toggle-camera').innerHTML = '<i data-feather="video"></i> Start Camera';
    feather.replace();
...
function startTracking() {
  ...
  // Update UI
  document.getElementById('toggle-tracking').innerHTML = '<i data-feather="eye-off"></i> Stop Tracking';
  feather.replace();
  ...
  eyeTrackingInterval = setInterval(async () => {
    if (videoElement.readyState === 4) {
      const eyeData = await processEyeTracking(videoElement, overlayCanvas, overlayContext);
  ...
  heartRateInterval = setInterval(async () => {
    if (videoElement.readyState === 4) {
      ...
      if (heartRateData) {
        ...
        document.getElementById('heart-rate-value').innerText = `${Math.round(lastHeartRate)} BPM`;
...
function stopTracking() {
  ...
  // Update UI
  document.getElementById('toggle-tracking').innerHTML = '<i data-feather="eye"></i> Start Tracking';
  feather.replace();
...
function addAvailableLaptop(laptopId) {
  ...
  if (laptopsList.querySelector('li')?.textContent.includes('No laptops available')) {
    laptopsList.innerHTML = '';
  }
...
function processPairRequest() {
  ...
  if (confirm(`Laptop (${laptopId.substring(0, 8)}...) wants to pair with your device. Accept?`)) {
    if (useWebSocket && ws && ws.readyState === WebSocket.OPEN) {
